...
export function AgentChatPanel() {
  ...
  const getStatusColor = (agentStatus: string) => {
    switch(agentStatus) {
      case 'complete': return 'text-emerald-500 bg-emerald-50 border-emerald-200';
      case 'error': return 'text-rose-500 bg-rose-50 border-rose-200';
      case 'waiting': return 'text-amber-500 bg-amber-50 border-amber-200';
  ...
  return (
    <div className="w-[420px] flex flex-col bg-white border-l border-[#eeeeee] h-full shadow-[-20px_0_40px_-20px_rgba(0,0,0,0.02)] transition-all animate-in slide-in-from-right-4 duration-500 relative z-20">
      
      {/* Header */}
      <div className="px-6 h-14 border-b border-[#f5f5f5] flex items-center justify-between bg-white shrink-0">
         <h3 className="text-[11px] font-black uppercase tracking-[0.25em] text-black font-inter flex items-center gap-2">
            AEGIS INTELLIGENCE 
            <span className="flex items-center gap-1.5 ml-2">
               <span className="text-[10px] font-bold text-emerald-600 tracking-widest">• ACTIVE</span>
            </span>
         </h3>
         <button 
           onClick={toggleChat}
           className="p-2 -mr-2 rounded-lg hover:bg-zinc-50 text-zinc-400 hover:text-black transition-all"
      ...
      {!isAuthorized && agentWalletAddress && (
        <div className="px-6 py-3 bg-amber-50 border-b border-amber-100 flex items-center justify-between animate-in slide-in-from-top duration-500">
           <div className="flex items-center gap-2">
              <ShieldCheck className="w-4 h-4 text-amber-600" />
              <div className="flex flex-col">
                 <span className="text-[10px] font-black text-amber-700 uppercase tracking-wider">Signature Required</span>
                 <span className="text-[9px] font-medium text-amber-600">Authorize platform to execute on-chain.</span>
              </div>
           </div>
           <button 
             onClick={async () => {
      ...
      {/* Conversation Thread */}
      <div className="flex-1 overflow-y-auto p-6 space-y-4 custom-scrollbar bg-[radial-gradient(#f8f8f8_1px,transparent_1px)] bg-[size:20px_20px]">
         {messages.length === 0 ? (
            ...
            messages.map((msg: ChatMessage, idx: number) => {
              ...
              if (msg.role === 'system') {
                return (
                  <div key={idx} className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-zinc-50 border border-zinc-100 animate-in fade-in duration-300">
                    <AlertTriangle className="w-3 h-3 text-zinc-400" />
                    <span className="text-[10px] font-bold text-zinc-500">{msg.content}</span>
                  </div>
                );
              }
                  ...
                  msg.role === 'user' ? "items-end" : "items-start"
                   ...
                   <div className={cn(
                      ...
                      msg.role === 'user' 
                         ...
                         : "bg-white text-zinc-800 border-[#eeeeee] rounded-tl-none"
                   )}>
                      {msg.content}
                   </div>
                   <span className="text-[9px] font-black uppercase tracking-widest text-zinc-400 px-1">
         ...
         {isBusy && (
            <div className="flex items-center gap-2 px-3 py-2 rounded-xl bg-white border border-[#eeeeee] w-fit shadow-sm">
      ...
      <div className="border-t border-[#f5f5f5] bg-white shadow-[0_-10px_20px_rgba(0,0,0,0.01)]">

         {/* Follow-up Questions (Typed Inputs) */}
         {showQuestions && (
            <div className="p-4 border-b border-[#f5f5f5] animate-in slide-in-from-bottom-2 duration-300">
                  ...
                  <div className="grid grid-cols-1 gap-3">
                     {structuredQuestions.map((q: StructuredQuestion) => (
                        <div key={q.field} className="space-y-1.5">
                           <label className="text-[9px] font-black text-amber-600/80 uppercase tracking-widest ml-1 flex items-center gap-1.5">
                              {q.input_type === 'address' && <span className="text-[8px] bg-amber-200/50 px-1.5 py-0.5 rounded">EVM</span>}
                              {q.input_type === 'url' && <span className="text-[8px] bg-amber-200/50 px-1.5 py-0.5 rounded">URL</span>}
                              {q.input_type === 'number' && <span className="text-[8px] bg-amber-200/50 px-1.5 py-0.5 rounded">NUM</span>}
                              {q.input_type === 'email' && <span className="text-[8px] bg-amber-200/50 px-1.5 py-0.5 rounded">EMAIL</span>}
                              {q.input_type === 'interval' && <span className="text-[8px] bg-amber-200/50 px-1.5 py-0.5 rounded">INTERVAL</span>}
                              {q.question}
                           </label>
                           {q.input_type === 'select' && q.options ? (
                              <select
                                className="w-full bg-white border border-amber-200 rounded-lg px-3 py-2 text-[11px] font-bold outline-none focus:border-amber-500 transition-all"
                              ...
                              <input
                                ...
                                className={cn(
                                  "w-full bg-white border border-amber-200 rounded-lg px-3 py-2 text-[11px] font-bold outline-none focus:border-amber-500 transition-all placeholder:text-amber-200",
         ...
         {showApproval && (
            <div className="p-4 border-b border-[#f5f5f5] animate-in slide-in-from-bottom-2 duration-300">
               <div className="bg-blue-50/50 border border-blue-100 rounded-2xl p-4 space-y-4">
                  ...
                  <p className="text-[11px] text-blue-700/70 font-medium">
                  ...
                  <div className="flex gap-2">
                     ...
                     <button 
                        ...
                        disabled={isBusy}
                        className="flex-1 bg-white hover:bg-zinc-50 disabled:opacity-50 text-zinc-700 border border-zinc-200 rounded-xl h-10 text-[10px] font-black uppercase tracking-widest transition-all flex items-center justify-center gap-2"
                     >
                        ✏️ Request Changes
                     </button>
                  </div>
         ...
         <div className="p-4">
            <div className="relative group">
               <textarea 
                  ...
                  className="w-full bg-zinc-50 border border-[#eeeeee] rounded-2xl p-4 pr-12 text-[12px] font-medium outline-none focus:border-black focus:bg-white transition-all resize-none min-h-[80px] shadow-sm group-hover:bg-white"
               />
               <button 
                  onClick={handleSend}
                  ...
                  className={cn(
                     ...
                     input.trim() && !isBusy 
                        ? "bg-black text-white hover:scale-110 active:scale-95 shadow-black/20" 
      ...
      <div className="p-4 border-t border-[#f5f5f5] bg-white">
         <button 
           onClick={() => setIsSettingsCollapsed(!isSettingsCollapsed)}
           className="w-full flex items-center justify-between px-2 mb-2 group"
         >
            <div className="flex items-center gap-2">
               <Settings className="w-3.5 h-3.5 text-zinc-400 group-hover:text-black transition-colors" />
               <span className="text-[10px] font-black text-zinc-400 group-hover:text-black uppercase tracking-widest transition-colors">Model Settings</span>
            </div>
            {isSettingsCollapsed ? <ChevronRight className="w-3.5 h-3.5 text-zinc-400" /> : <ChevronDown className="w-3.5 h-3.5 text-zinc-400" />}
         </button>
            ...
            <div className="space-y-4 px-2 py-2 animate-in slide-in-from-top-2 duration-300">
               <div className="flex items-center gap-4">
                  <div className="flex-1 space-y-1.5">
                     <label className="text-[9px] font-black text-zinc-400 uppercase tracking-widest">Planning Engine</label>
                     <select 
                       value={planningModel}
                       onChange={(e) => setModels(e.target.value, codegenModel)}
                       className="w-full bg-zinc-50 border border-[#eeeeee] rounded-lg px-3 py-2 text-[11px] font-bold outline-none focus:border-black transition-all"
                     >
                  ...
                  <div className="flex-1 space-y-1.5">
                     <label className="text-[9px] font-black text-zinc-400 uppercase tracking-widest">Code Gen</label>
                     <select 
                       value={codegenModel}
